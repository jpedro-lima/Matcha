...
import type { LoginResult, RegisterInput } from '../models/auth.js'
import { publicUserFromRow, type PublicUser, type UserRow } from '../models/user.js'
...
// Re-exporta os DTOs para compatibilidade com consumidores existentes que
// importam direto de `auth-service`. Novos imports devem vir de `models/`.
export type { LoginResult, RegisterInput } from '../models/auth.js'
export type { PublicUser } from '../models/user.js'

// Subset minimal da linha do banco que cada operação precisa — evita SELECT *.
type AuthRow = Pick<
	UserRow,
...
// Sem blocklist persistente nesta fase (planning permite); o controller apaga
// o cookie no cliente. Mantemos a função aqui para o controller chamar e
// para evoluirmos para uma blocklist quando virar prioridade (Fase 10).
...
export async function forgotPassword(email: string): Promise<void> {
	...
	if (!user) return // silencioso (anti-enumeration)
...
// Substitui a query direta que estava no controller (`auth-controller.meHandler`).
// Mantém a regra "controllers não tocam knex".
